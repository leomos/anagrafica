...
public class ClienteValidator implements Validator {
	...
	public void validate(Object cliente, Errors e) {
		...
		if (Optional.ofNullable(cli.getDataDiNascita()) == null || !Optional.ofNullable(cli.getDataDiNascita()).isPresent()) {

			e.rejectValue("dataDiNascita", "data di nascita nulla");
		}

		if (Optional.ofNullable(cli.getLuogoDiNascita()) == null || Optional.ofNullable(cli.getLuogoDiNascita()).get().isEmpty()) {
...
if(Optional.ofNullable(cli.getSesso()).isPresent()) {
		if (!Optional.ofNullable(cli.getSesso()).get().equals('M') && !Optional.ofNullable(cli.getSesso()).get().equals('m')
				&& !Optional.ofNullable(cli.getSesso()).get().equals('f') && !Optional.ofNullable(cli.getSesso()).get().equals('F')
		...
		for ( int i=0; i<c.length-1;i++) {
			...
			try {
				...
				e.rejectValue("telefono", " il carattere del telefono inserito : " + ca + " deve essere un numero");
				}
			}
		}
